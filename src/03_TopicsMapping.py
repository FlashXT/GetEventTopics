...
def train_w2v_Model(etopicpath,gtopicpath,modelpath):
    ...
    model = Word2Vec(text1, window=2, min_count=1,workers=multiprocessing.cpu_count())
...
#计算两个文档的相似度
def topicSimilarity(modelpath,etopicpath,gtopicpath):
    model = gensim.models.Word2Vec.load(modelpath)
    # model = KeyedVectors.load_word2vec_format(modelpath,binary=True)
    path = os.getcwd() + "\\..\\Data\\DestData\\WordSimilarity.csv"
    ...
    [gtopics.append([item[2],item[3],processText(item[3])]) for item in gt]

    list = []
    for item in etopics:
        sim={'key':None,'value':0}
        for topic in gtopics:
            # print(item[0], item[1], '<--->', topic, '\t',model.n_similarity(item[2].split(" "), topic[2]))
            try:
                simvalue = model.n_similarity(item[2].split(" "), topic[2])
...
def main():
    ...
    plotpath = os.getcwd()+"\\..\\Data\\DestData\\WordSimilarity2C.csv"

    # train_w2v_Model(datapath,gtopicpath,modelpath)
    topicSimilarity(modelpath, etopicpath,gtopicpath)
    # PlotTopics(plotpath)
    # return 0
